...
try {
    ...
    // Aiven requires SSL, so we disable SSL verification to allow connection
    // from Render without needing to download and configure the CA certificate file.
    if (getenv('DB_HOST') && strpos(getenv('DB_HOST'), 'aivencloud') !== false) {
